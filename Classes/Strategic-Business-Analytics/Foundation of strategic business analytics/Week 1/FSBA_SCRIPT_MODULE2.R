...
dir <- choose.dir()
data=read.table(file.path(dir,'DATA_2.01_SKU.csv'), header = T,sep=',') # The function read.table enables us to read flat files such as .csv files
...
data=read.table(file.path(dir,'DATA_2.02_HR.csv'),header = T,sep=',') # The function read.table enables us to read flat files such as .csv files
...
d = dist(testdata[,1:3], method = "euclidean") # the dist() function computes the distances of all the observations in our dataset
hcward = hclust(d, method="ward.D") # hclust() function performs hiearchical clustering, we pass it the distances, and we set the method argument to "ward.D"
...
# Let's load the data
data=read.table(file.path(dir,'DATA_2.03_Telco.csv'), header = T,sep=',')# The function read.table enables us to read flat files such as .csv files
