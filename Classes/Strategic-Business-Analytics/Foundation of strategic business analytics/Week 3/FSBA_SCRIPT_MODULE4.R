...
dir <- choose.dir()
dataold=read.table(file.path(dir,'DATA_3.01_CREDIT.csv'),sep=',',header=TRUE) # The function read.table enables us to read flat files such as .csv files
datanew=read.table(file.path(dir,'DATA_4.01_CREDIT2.csv'),sep=',',header=TRUE) # The function read.table enables us to read flat files such as .csv files
...
dataold=read.table(file.path(dir,'DATA_3.02_HR2.csv'), header = T,sep=',') # The function read.table enables us to read flat files such as .csv files
datanew=read.table(file.path(dir,'DATA_4.02_HR3.csv'), header = T,sep=',') # The new dataset on which we want to make the prediction
...
head(predattrition) # View the predattrition dataframe
...
# Let's load the data
data=read.table(file.path(dir,'DATA_4.03_MNT.csv'),sep=',',header=TRUE)
...
head(Forecast) # View the complete Forecast dataframe

Forecast=Forecast[order(Forecast$RemainingLT, decreasing = T),] # Order the elements by Expected Remaining Lifetime
ActionsPriority=Forecast[Forecast$broken==0,] # And keep only those who are not broken yet
head(ActionsPriority) # View the output and take actions!
...
# Let's load our dataset and call it data
data=read.table(file.path(dir,'DATA_4.04_CHOC.csv'),sep=',',header=TRUE) # The function read.table enables us to read flat files such as .csv files
