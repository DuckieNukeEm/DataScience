...
%%%% running through the itterations to get to the final sigmoid function
#a1 = [ones(m, 1) X];
#a2 = [ones(m, 1) sigmoid(a1 * Theta1')];
#sig = sigmoid(a2 * Theta2');

%%% restructing y so that it's a vector of 0 or 1,
#yv = repmat([1:num_labels], m, 1) == repmat(y, 1, num_labels);

#cost = -1/m * log(sig)' * yv + log(1-sig)' * (1-yv);
#J = cost;
#Theta1NoBias = Theta1(:, 2:end); %taking out the blank 0 at the start
#Theta2NoBias = Theta2(:, 2:end); %taking out the blank 0 at the start
